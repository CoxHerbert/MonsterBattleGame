...
export class Renderer {
  ...
  private drawPath(level: LevelConfig) {
    this.pathG.clear()
    this.pathG.lineStyle(4, 0xffff00, 1)
    for (const p of level.paths) {
      const wps = p.waypoints
      this.pathG.moveTo(wps[0].x, wps[0].y)
      for (let i = 1; i < wps.length; i++) this.pathG.lineTo(wps[i].x, wps[i].y)
    }
  }
  ...
  drawTowers(towers: Tower[]) {
    this.syncPool(
      ...
      (g, t) => {
        ...
        gg.clear()
        gg.beginFill(0x00ff00)
        gg.drawRect(-16, -16, 32, 32)
        gg.endFill()
        gg.position.set(t.x, t.y)
      }
    )
  }
